 ...
 * an end-user only.
 *
 * File: gm_model_biquad_wfa_interface_old.c
 *
...
#include "gm_model_biquad_wfa_interface_old.h"
#include "gm_model_biquad_wfa_interface_old_private.h"
#include "gm_model_biquad_wfa_interface_old_dt.h"
...
static struct iio_channel* gm_model_biquad__base_getChan_m(struct iio_device*
  iioDev, boolean_T output);
static void gm_model_biqu_base_doTeardown_m(matlabshared_libiio_axistre_m_T *obj);
static void gm_model_biqua_SystemCore_setup(matlabshared_libiio_axistream_T *obj);
static void gm_model_biq_SystemCore_setup_m(matlabshared_libiio_axistre_m_T *obj);
static struct iio_channel* gm_model_biquad_wf_base_getChan(struct iio_device*
  iioDev, boolean_T output)
{
...
static void gm_model_biqu_base_doTeardown_m(matlabshared_libiio_axistre_m_T *obj)
{
  ...
  if (obj->needsTeardown) {
    ...
    obj->bufExists = false;
    gm_model_biquad_wfa_interface_B.chanPtr_o = gm_model_biquad__base_getChan_m
      (obj->iioDev, true);
...
static void gm_model_biq_SystemCore_setup_m(matlabshared_libiio_axistre_m_T *obj)
{
  ...
  gm_model_biquad_wfa_interface_B.chanPtr_j = gm_model_biquad__base_getChan_m
    (obj->iioDev, true);
  status = check_pointer(gm_model_biquad_wfa_interface_B.chanPtr_j);
  if (status == 0) {
  ...
  if (obj->buffersAvailable) {
    ...
    if (!isScan) {
      gm_model_biqu_base_doTeardown_m(obj);
      for (status_0 = 0; status_0 < 11; status_0++) {
...
/* Model step function */
void gm_model_biquad_wfa_interface_old_step(void)
{
  ...
  AXI4LITE_WRITE(gm_model_biquad_wfa_interfac_DW.obj_b.DeviceState,
  ...
  AXI4LITE_WRITE(gm_model_biquad_wfa_interfac_DW.obj_l.DeviceState,
...
void gm_model_biquad_wfa_interface_old_initialize(void)
{
  /* Registration code */

  /* initialize non-finites */
  rt_InitInfAndNaN(sizeof(real_T));
  rtmSetTFinal(gm_model_biquad_wfa_interfac_M, -1);
  gm_model_biquad_wfa_interfac_M->Timing.stepSize0 = 0.001;

  /* External mode info */
  gm_model_biquad_wfa_interfac_M->Sizes.checksums[0] = (1094930905U);
  gm_model_biquad_wfa_interfac_M->Sizes.checksums[1] = (2097368922U);
  gm_model_biquad_wfa_interfac_M->Sizes.checksums[2] = (3679704523U);
  gm_model_biquad_wfa_interfac_M->Sizes.checksums[3] = (4280589581U);
...

  {
...

    {
      ...
      {
        static real_T pDataValues0[] = { 1.0, 0.70710678118654757,
          6.123233995736766E-17, -0.70710678118654746, -1.0,
          -0.70710678118654768, -1.8369701987210297E-16, 0.70710678118654735,
          1.0, 0.70710678118654768, 3.0616169978683831E-16, -0.70710678118654668,
          -1.0, -0.70710678118654713, -4.2862637970157361E-16,
          0.70710678118654657, 1.0, 0.70710678118654857, 2.3274478990165594E-15,
          -0.70710678118654646, -1.0, -0.70710678118654735,
          -2.4499125789312946E-15, 0.70710678118654646, 1.0, 0.7071067811865499,
          2.57237725884603E-15, -0.70710678118654635, -1.0, -0.70710678118654757,
          -2.6948419387607653E-15, 0.70710678118654624 } ;
    ...
    gm_model_biquad_wfa_interfac_DW.obj_b.matlabCodegenIsDeleted = true;
    gm_model_biquad_wfa_interfac_DW.obj_b.isInitialized = 0;
    gm_model_biquad_wfa_interfac_DW.obj_b.matlabCodegenIsDeleted = false;
    obj = &gm_model_biquad_wfa_interfac_DW.obj_b;
    gm_model_biquad_wfa_interfac_DW.obj_b.isSetupComplete = false;
    gm_model_biquad_wfa_interfac_DW.obj_b.isInitialized = 1;
    gm_model_biquad_wfa_interfac_DW.obj_b.DeviceState = NULL;
    for (i = 0; i < 14; i++) {
      gm_model_biquad_wfa_interface_B.b_da[i] = tmp[i];
    }

    obj->DeviceState = AXI4LITE_INIT(&gm_model_biquad_wfa_interface_B.b_da[0]);
    gm_model_biquad_wfa_interfac_DW.obj_b.isSetupComplete = true;

    /* End of Start for MATLABSystem: '<S5>/AXI4-Interface Write' */

    /* Start for MATLABSystem: '<S4>/AXI4-Interface Write' */
    gm_model_biquad_wfa_interfac_DW.obj_l.matlabCodegenIsDeleted = true;
    gm_model_biquad_wfa_interfac_DW.obj_l.isInitialized = 0;
    gm_model_biquad_wfa_interfac_DW.obj_l.matlabCodegenIsDeleted = false;
    obj = &gm_model_biquad_wfa_interfac_DW.obj_l;
    gm_model_biquad_wfa_interfac_DW.obj_l.isSetupComplete = false;
    gm_model_biquad_wfa_interfac_DW.obj_l.isInitialized = 1;
    gm_model_biquad_wfa_interfac_DW.obj_l.DeviceState = NULL;
    for (i = 0; i < 14; i++) {
      gm_model_biquad_wfa_interface_B.b_da[i] = tmp[i];
    }

    obj->DeviceState = AXI4LITE_INIT(&gm_model_biquad_wfa_interface_B.b_da[0]);
    gm_model_biquad_wfa_interfac_DW.obj_l.isSetupComplete = true;
    ...
    gm_model_biq_SystemCore_setup_m(&gm_model_biquad_wfa_interfac_DW.obj_o);
  }
}

/* Model terminate function */
void gm_model_biquad_wfa_interface_old_terminate(void)
{
  ...
  if (!gm_model_biquad_wfa_interfac_DW.obj_o.matlabCodegenIsDeleted) {
    ...
    if ((gm_model_biquad_wfa_interfac_DW.obj_o.isInitialized == 1) &&
      ...
      usleep(1000U);
      gm_model_biqu_base_doTeardown_m(&gm_model_biquad_wfa_interfac_DW.obj_o);
    }
  }
  ...
  if (!gm_model_biquad_wfa_interfac_DW.obj_b.matlabCodegenIsDeleted) {
    gm_model_biquad_wfa_interfac_DW.obj_b.matlabCodegenIsDeleted = true;
    if ((gm_model_biquad_wfa_interfac_DW.obj_b.isInitialized == 1) &&
        gm_model_biquad_wfa_interfac_DW.obj_b.isSetupComplete) {
      AXI4LITE_TERMINATE(gm_model_biquad_wfa_interfac_DW.obj_b.DeviceState);
    }
  }
  ...
  if (!gm_model_biquad_wfa_interfac_DW.obj_l.matlabCodegenIsDeleted) {
    gm_model_biquad_wfa_interfac_DW.obj_l.matlabCodegenIsDeleted = true;
    if ((gm_model_biquad_wfa_interfac_DW.obj_l.isInitialized == 1) &&
        gm_model_biquad_wfa_interfac_DW.obj_l.isSetupComplete) {
      AXI4LITE_TERMINATE(gm_model_biquad_wfa_interfac_DW.obj_l.DeviceState);
    }
  }
